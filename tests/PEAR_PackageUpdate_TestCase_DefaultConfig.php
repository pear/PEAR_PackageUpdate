...
class PEAR_PackageUpdate_TestCase_DefaultConfig extends PHPUnit_Framework_TestCase
{
    ...
    public static function main()
    {
        ...
        $suite = new PHPUnit_Framework_TestSuite("PEAR_PackageUpdate test default configuration");
    ...
    protected function setUp()
    {
        ...
        require_once '..' . DIRECTORY_SEPARATOR . 'PackageUpdate.php';
     ...
     *
     * Will use the pear config files into the default system directory (PEAR_CONFIG_SYSCONFDIR).
     *
     * @access public
     * @since  1.0.0
     */
    public function testWrongDriver()
    {
        ...
        $this->assertFalse($ppu instanceof PEAR_PackageUpdate, 'Null driver exist in the include path');
    }
    ...
    {
        $pref_file = dirname($this->pref_file) . DIRECTORY_SEPARATOR . 'wrong.ppurc.ini';
        $ppu =& PEAR_PackageUpdate::factory('Cli', 'Console_Getopt', 'pear', '', '', $pref_file);
        if ($ppu !== false) {
            $e = $ppu->popError();
            $this->assertTrue(($e['code'] == PEAR_PACKAGEUPDATE_ERROR_INVALIDINIFILE)
    ...
    {
        $pref_file = dirname($this->pref_file) . DIRECTORY_SEPARATOR . 'corrupted.ppurc.ini';
        ...
        if ($ppu !== false) {
            $e = $ppu->popError();
            $this->assertTrue($e['code'] == PEAR_PACKAGEUPDATE_ERROR_PREFFILE_CORRUPTED);
        }
    }
}

// Call PEAR_PackageUpdate_TestCase_DefaultConfig::main() if this source file is executed directly.
